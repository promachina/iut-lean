...
\item the Step (xii) shadow: local Frobenioid \(p_v^N\)-ambiguity changes
  ...
  both the fixed q/determinant boundary and a nonzero local shift, Lean proves
  that the quotient identifies them while the underlying log-volumes remain
  unequal.  The same diagnostic applies to two shifted local readings with
  unequal exponents and nonzero local step when both lie in the collapsed
  subset;
