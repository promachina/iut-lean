...
the nonpositive/nonnegative branch hypotheses.  It can also consume the
decomposed zero/nonzero input directly.  For nonzero labels, Lean proves the
complementary bound: the absolute exponent is at least \(1\), so a nonpositive
environment degree below the \(\Theta\)-source average bounds every nonzero
Gaussian component and hence their finite nonzero-label average.  The
