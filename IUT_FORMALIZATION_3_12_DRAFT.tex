...
\item the IUT II Remark 4.7.3 zero/nonzero weighted-volume relation:
  ...
  descends to full absolute labels if and only if \(t=0\).
  Lean separately proves that raw \(F_\ell\)-coordinate sums are invariant
  under additive translation; this is only finite-permutation invariance and
  does not supply descent to \(|F_\ell|\).  For the Gaussian coordinate
  average, Lean packages this as a diagnostic: every nonzero additive
  translation preserves the raw coordinate average but fails full-label descent.
