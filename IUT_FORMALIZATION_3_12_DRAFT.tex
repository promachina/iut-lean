...
\item an absolute-label exponent on \(|F_\ell|=\{0\}\cup F_\ell^\times/\{\pm1\}\)
  ...
  that the zero absolute label is reached exactly by \(j=0\), and that every
  nonzero sign-label class has a nonzero coordinate representative; Lean also
  proves that the half-range procession map onto \(|F_\ell|\) is surjective,
  using the minimal absolute representative \(j_{\min}\);
  ...
  \(6\,\operatorname{Avg}(G)=j_{\max}(2j_{\max}+1)\deg(q)\);
...
\item the zero/nonzero Gaussian average split:
  ...
  by the auxiliary nonzero-only average;
\item the IUT II Remark 4.7.3 zero/nonzero weighted-volume relation:
  ...
  the zero label, proves that \(0\ll0\) is false, and proves that this relation
  is not symmetric at the zero label; at coordinate level, every nonzero
  \(j\in F_\ell\) is subordinate to zero, while the corresponding all-coordinate
  statement is false because it includes \(j=0\); Lean also converts the
...
\(\ell-1\), the zero Gaussian degree is \(0\), and the all-\(F_\ell\) average is
\((\ell-1)/\ell\) times the nonzero average.  The remaining task is to replace
