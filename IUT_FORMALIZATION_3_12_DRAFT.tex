...
\item the IUT II Remark 4.7.3 zero/nonzero weighted-volume relation:
  ...
  subordinate predicate for all \(j\) if and only if \(t=0\).
  ...
  representative-square transport branch.
...
Before extending the mathematics, the Lean tree should be trimmed so that the
root import exposes the present corridor rather than early toy scaffolding.  This
cleanup is organizational only: the continuation point after it remains the
finite \(q^{j^2}\) branch just reached above, namely the exact theorem that
nondegenerate affine pointwise Gaussian preservation is equivalent to zero
translation plus sign action.
