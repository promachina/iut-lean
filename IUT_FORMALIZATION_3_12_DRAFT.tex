...
\item the finite procession containers
  ...
  and stage cardinality \(j+1\);
\item the log-volume shadow of the tensor packet over \(S^\pm_{j+1}\), with
  total log-volume given by the finite sum over the container and normalized
  denominator \(j+1\);
...
\texttt{processionContainerSkeletonAvailable} & = \texttt{true},\\
\texttt{processionTensorPacketLogVolumeAvailable} & = \texttt{true},\\
