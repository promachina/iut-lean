...
\item the IUT IV, Corollary 2.2(ii) condition (C1) prime-scale window:
  \((\log(q_\forall))^{1/2}\le\ell\le
   10\delta(\log(q_\forall))^{1/2}\).  Lean keeps the square-root term explicit,
  proves \(\log(q_\forall)\ge0\), and derives \(1\le10\delta\) from the nonempty
  window;
...
three-step logarithmic inequality chain.  The \(\epsilon_E\)-definition flag
records the source formula and lower estimate for \(\epsilon_E\).  The
\(\epsilon_E\)-absorption flag records the final denominator removal in C2.  The
log-different/log-conductor flag records the bridge from tripodal field terms to
curve functions.  The Corollary 2.2-to-Theorem A flag records the formal
...
\((\log(q_\forall))^{1/2}\), and \(1\le10\delta\).
