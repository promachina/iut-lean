...
square-weight transport data feeds such a comparison, while several simplified
collapse mechanisms are rejected by Lean.
\end{abstract}
...
\item the Step (x)-sourced statement endpoint:
  ...
  Corollary 3.12 statement endpoint, with finite \(\Theta\)-branch and
  \(C_\Theta\ge-1\); Lean proves that this constructed endpoint still records
  the \(\Theta\)-pilot as subject to \((\mathrm{Ind1}),(\mathrm{Ind2}),
  (\mathrm{Ind3})\) and the \(q\)-pilot as not subject to them; Lean also proves
  ...
  these equal averages; Lean also proves
  ...
  the finite \(\Theta\)-value is identified with the Step (x)
  \((\mathrm{Ind3})\) upper bound; the same endpoint bound holds after the
...
action, canonical positive square-weight profile, the constant-average check,
...
endpoint, identification of its \(q\)-pilot real with the Step (x)
pre-, after-\((\mathrm{Ind1})\)-, and after-\((\mathrm{Ind2})\)-averages, and direct
\(-|\log(q)|\le-|\log(\Theta)|\) inequality,
the
