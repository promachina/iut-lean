...
def m1m3PaperLedger : List PaperClause :=
  [ clause "FrdI.1.1(i-ii)" .frobenioidsI
    ...
    clause "III.3.1" .iutIII "IUT III, Proposition 3.1"
      ...
      .partialImplementation
      "Rational places are literal places of Q; selected fibers v above v_Q are derived and only those fibers are required finite. Local logarithmic modules are filtered colimits over the actual completion Q_(v_Q). A finite-place constructor now has the literal v-adic completion K_v as carrier. In accordance with Proposition 3.1 and the local-degree normalization of Remark 3.1.1(ii), its Q_p extension obligation is an algebra with continuous scalar multiplication, not a norm-nonincreasing NormedAlgebra: the source normalizes the two absolute values differently. Mathlib's tensor-product/dense-range completion theorem derives module-finiteness and finite-dimensionality from exactly this topological-module interface. A global uniformizer proves the completion nontrivially normed; the rational completion is locally compact via its continuous equivalence with Q_p; finite-dimensional topological local compactness and the intrinsic norm then prove K_v proper. Countability of the number field derives separability and second countability. Thus no independent topology or replacement-field obligation remains. A separate field presentation exhibits every stage as a finite-dimensional complete locally compact local field and every transition as a continuous field embedding commuting with the module diagram; identity and composition laws are derived. Algebraic restriction to Q remains separate from topology. Constructing the canonical continuous completion algebra action, instantiating every field stage, constructing these presentations and the log-shell algorithm from Proposition 1.2, and proving every Proposition 3.1 poly-isomorphism remain open.",
