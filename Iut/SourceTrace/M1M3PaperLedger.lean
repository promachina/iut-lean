...
def m1m3PaperLedger : List PaperClause :=
  [ clause "FrdI.1.1(i-ii)" .frobenioidsI
    ...
    clause "III.3.1" .iutIII "IUT III, Proposition 3.1"
      ...
      .partialImplementation
      "Rational places are literal places of Q; selected fibers v above v_Q are derived and only those fibers are required finite. Local logarithmic modules are filtered colimits over the actual completion Q_(v_Q). A finite-place constructor now has the literal v-adic completion K_v as carrier. In accordance with Proposition 3.1 and the local-degree normalization of Remark 3.1.1(ii), the contracted rational valuation is proved equivalent to the restriction of the K-valuation, rather than equal to it. This equivalence and a cofinal uniformizer argument construct the uniformly continuous dense rational map and its unique continuous extension Q_p -> K_v. The resulting algebra, continuous scalar action, and Q -> Q_p -> K_v tower are derived, so no completion-extension structure is accepted as input. Mathlib's tensor-product/dense-range theorem then derives module-finiteness and finite-dimensionality. A global uniformizer proves the completion nontrivially normed; the rational completion is locally compact via its continuous equivalence with Q_p; finite-dimensional topological local compactness and the intrinsic norm prove K_v proper. Countability of the number field derives separability and second countability. Thus no independent topology, replacement field, or completion algebra obligation remains. At a nonarchimedean rational place, the finite Galois intermediate subfields of the spectral-normed algebraic closure now form the actual filtered diagram. Its transitions are continuous field inclusions, every stage is a complete locally compact second-countable local field, and the final TopModuleCat cocone is proved colimiting. The induced map from the direct limit to the algebraic closure is proved injective and surjective. Krull theory proves that every stage is exactly the fixed field of an open subgroup. The actual selected completion K_v embeds continuously into the closure, and a finite image basis constructs a stage containing all of K_v. Thus the finite-place field presentation is constructed rather than accepted as data. Algebraic restriction to Q remains separate from topology. Constructing the archimedean presentation, connecting these canonical modules functorially to every holomorphic prime strip, constructing the log-shell algorithm from Proposition 1.2, and proving every Proposition 3.1 poly-isomorphism remain open.",
