...
def m1m3PaperLedger : List PaperClause :=
  [ clause "FrdI.1.1(i-ii)" .frobenioidsI
    ...
    clause "III.3.1" .iutIII "IUT III, Proposition 3.1"
      ...
      .partialImplementation
      "Rational places are literal places of Q; selected fibers v above v_Q are derived and only those fibers are required finite. Local logarithmic modules are filtered colimits over the actual completion Q_(v_Q). A finite-place constructor now has the literal v-adic completion K_v as carrier. Its canonical Q_p normed algebra and scalar-tower law remain explicit Mathlib integration obligations, while Mathlib's tensor-product/dense-range completion theorem derives module-finiteness and finite-dimensionality. Properness and second countability remain exact topology obligations rather than properties of a replacement field. A separate field presentation exhibits every stage as a finite-dimensional complete locally compact local field and every transition as a continuous field embedding commuting with the module diagram; identity and composition laws are derived. Algebraic restriction to Q remains separate from topology. Instantiating every field stage by the canonical completion, constructing these presentations and the log-shell algorithm from Proposition 1.2, and proving every Proposition 3.1 poly-isomorphism remain open.",
