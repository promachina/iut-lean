...
def m1m3PaperLedger : List PaperClause :=
  [ clause "FrdI.1.1(i-ii)" .frobenioidsI
    ...
    clause "III.3.11(iii).horizontal" .iutIII "IUT III, Theorem 3.11(iii): horizontal theta link"
      ...
      .partialImplementation
      "The active boundary is an adjacent-column corridor, not a field of one column. It proves that the columns use the same lattice and have indices n and n+1. Following IUT I Section 0, an ordinary F-prime-strip full poly-isomorphism is the quotient of componentwise equivalence maps by natural isomorphism on categorical components, not a strict categorical Iso of prime-strip records. Quotient identity, composition, associativity, and inverse-pair cancellation are proved. The current ordinary-F precursor is a fixed-corner square for every m at the site/common lattice endpoints. Its selected arrows witness nonemptiness, while two universal fields prove class-by-class compatibility in both directions; its environment precursor is derived by conjugation. This does not yet implement clauses (a) and (b): Theorem 3.11 uses the Definition 4.9 F-turnstile-times-mu prime-strips. Exact finite and archimedean construction interfaces now exist, but their derivation from Frobenioid evaluation/reconstruction, population at every endpoint, prime-strip isomorphism category, Theorem 1.5 horizontal link, and Corollary 2.3 environment transport remain open. Clause (c) is a square of actual mono-theta projective systems for every m and selected bad place. Clause (d) is a kappa-sol/M-infinity-kappa square for every m and absolute label, plus the common square and coherence with both vertical labeled Kummer maps. Evaluation equivariance also remains open.",
