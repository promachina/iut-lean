  ...
  [ clause "FrdI.1.1(i-ii)" .frobenioidsI
    ...
    clause "GeoAn.1.1.4" .geometryAnabelioids
      ...
      ["SourcePointedAnabelioidHom.TwoIso",
        ...
        "SourcePointedAnabelioidEquivalence",
        "SourcePointedAnabelioidEquivalence.inversePointedHom",
        "SourcePointedAnabelioidEquivalence.fundamentalGroupHom_bijective",
        "SourcePointedAnabelioidEquivalence.fundamentalGroupContinuousMulEquiv"]
      .partialImplementation
      "For two pointed morphisms whose exact pullback functors are naturally isomorphic, Lean derives the basepoint discrepancy in Aut(fiber), reconstructs the corresponding certified target fundamental-group element, and proves that the two induced homomorphisms differ by its inner conjugation. For a pointed exact categorical equivalence, Lean constructs the quasi-inverse pointed morphism from the categorical inverse, obtains both triangle 2-isomorphisms, proves bijectivity of the derived fundamental-group map by cancelling the resulting inner conjugations, and upgrades it to a continuous profinite-group equivalence. The converse construction and the full equivalence between the category of morphisms and continuous homomorphisms modulo conjugation remain open.",
    clause "GeoAn.1.2.1-1.2.2.1" .geometryAnabelioids
      ...
      ["continuousFiniteAction",
        ...
        "sourceDependentSection_continuous",
        ...
        "SourceFiniteEtaleFunctorFactorization.connectedObjectOpenStabilizer",
        "SourceFiniteEtaleFunctorFactorization.connectedObjectCosetActionIso",
        "SourceFiniteEtaleFunctorFactorization.connectedSourceEquivalence",
        "SourceFiniteEtaleFunctorFactorization.connectedSourcePullbackIsoRestriction",
        "SourceEquivalenceBasepoint.etaleFundamentalGroup",
        "SourceFiniteEtaleFunctorFactorization.connectedObjectProfiniteGroup",
        "SourceFiniteEtaleFunctorFactorization.connectedObjectProfiniteInclusion",
        "SourceFiniteEtaleFunctorFactorization.connectedSourceEtaleFundamentalGroup",
        "SourceFiniteEtaleFunctorFactorization.connectedPointedHom",
        "SourceFiniteEtaleFunctorFactorization.connectedPointedHom_fundamentalGroupHom_eq",
        "SourceFiniteEtaleFunctorFactorization.connectedObjectProfiniteInclusion_isOpenEmbedding",
        "SourceFiniteEtaleFunctorFactorization.connectedFiniteEtaleHom",
        "SourceFiniteEtaleFunctorFactorization.connectedFiniteEtaleHom_fundamentalGroupHom_eq"]
      .partialImplementation
      "Lean constructs B(G) as the actual Galois category of finite discrete continuous G-actions and certifies its original profinite fundamental group. For every open H ≤ G it constructs the transitive action G/H, the paper's left-diagonal quotient and right-inverse G-action, the equivalent standard induced action, and the natural equivalence B(H) ≃ B(G)_(G/H). The forgetful slice functor is proved left adjoint to product with S. For arbitrary S, Lean constructs the finite continuous conjugation action on dependent sections, proves the explicit currying equivalence natural in both variables, and derives that product with S is also a left adjoint. Hence i_S^* preserves finite limits and finite colimits exactly as Proposition 1.2.1(iii) requires. It also constructs the finite orbit set of S, restriction and gluing functors, both natural inverse isomorphisms, and the resulting equivalence between B(G)_S and the product of its orbit slices. Orbit-stabilizer then identifies every factor with B(Stab(s)) for an explicitly open stabilizer and supplies its connected Galois-category certificate, proving Proposition 1.2.1(i) in the continuous-action model. Explicit inverse Hom maps prove induction left adjoint to restriction; uniqueness of right adjoints identifies restriction with product by G/H under the slice equivalence. The resulting pullback is packaged by the factorization definition of finite-etale morphisms. Conversely, if the source of such a factorization is a connected Galois category, Lean proves the slicing action nonempty and transitive, recovers its open stabilizer H, constructs S ≅ G/H and the source equivalence with B(H), and proves that the original pullback becomes restriction along H ≤ G. It then performs Remark 1.2.2.1's choice of appropriate basepoints by transporting the canonical B(H) fiber functor back across this equivalence, certifies H as the source fundamental group, equips the original pullback with the resulting pointed structure, proves its derived profinite-group map is literally H ↪ G, proves this map is an open embedding, and packages the connected finite-etale morphism. The arbitrary-fiber-functor comparison remains part of the broader Proposition 1.1.4 generality, not an obligation of this chosen-basepoint converse.",
    ...
    clause "I.5.2(v)" .iutI "IUT I, Definition 5.2(v)"
      ["SourceTopologicalPseudoMonoid",
        ...
        "SourceFinitePlaceReconstruction.module",
        "SourceFinitePlaceReconstruction.carrierEquivAlgebraicClosure",
        "SourceFinitePlaceReconstruction.continuous_stageIntegralToInd",
        "SourceFinitePlaceReconstruction.exists_stage_representation",
        "SourceFinitePlaceReconstruction.indIntegralMonoidEquivSourceMLF",
        "SourceFinitePlaceReconstruction.indIntegralMonoidEquivSourceMLF_galois",
        "SourceFinitePlaceReconstruction.galoisContinuousLinearMap",
        "SourceFinitePlaceReconstruction.carrierMulEquiv_galoisContinuousLinearMap",
        "SourceFinitePlaceReconstruction.continuous_indGaloisMulAut",
        "SourceFinitePlaceReconstruction.indGaloisContinuousMulEquiv",
        "SourceFinitePlaceReconstruction.indGaloisContinuousMulEquiv_one",
        "SourceFinitePlaceReconstruction.indGaloisContinuousMulEquiv_mul",
        "SourceFinitePlaceReconstruction.continuous_finiteStageGaloisEvaluation",
        "SourceFinitePlaceReconstruction.continuous_stageGaloisAction",
        "SourceFinitePlaceReconstruction.continuous_galoisLinearMap_comp_stageToCarrier",
        "SourceFinitePlaceReconstruction.continuous_indGaloisAction_comp_stageIntegralToInd",
        "SourceFinitePlaceReconstruction.stageIntegralTransition_trans",
        "SourceFinitePlaceReconstruction.continuous_stageGaloisIntegralAction",
        "SourceFinitePlaceReconstruction.stageIntegralTransition_galois",
        "SourceFinitePlaceReconstruction.integralIndSystem",
        "SourceFinitePlaceReconstruction.stageIntegralToIndHom_transition",
        "SourceFinitePlaceReconstruction.stageIntegralToIndHom_galois",
        "SourceFinitePlaceReconstruction.integralIndSystemLimit",
        "SourceFinitePlaceReconstruction.integralIndSequentialPresentation",
        "SourceFinitePlaceReconstruction.denseRange_algebraMap_base",
        "SourceFinitePlaceReconstruction.exists_stage_primitive_generator",
        "SourceFinitePlaceReconstruction.exists_global_polynomial_root_same_adjoin",
        "SourceFinitePlaceReconstruction.exists_global_polynomial_root_generating_stage",
        "SourceFinitePlaceReconstruction.globalPolynomialRootStage_surjective",
        "SourceFinitePlaceReconstruction.stageIndexCountable",
        "SourceFinitePlaceReconstruction.integralIndSourceSequentialPresentation",
      ...
      .partialImplementation
      "The exact Section 0 pseudo-monoid and continuous action-pair targets are available. The D_v group is correctly the X_v arithmetic fundamental group, and the actual sign-quotient geometry constructs its open embedding into the C_v group, with injectivity and open image proved. For every actual number-field completion K_v, the algebraic closure is constructed as the filtered topological-module colimit of its finite Galois local subfields; its nonzero integral submonoid is exhausted by continuous finite-stage maps and identified with O_(Kbar_v)^triangle. Restriction to every finite Galois stage induces a continuous colimit automorphism; it is proved equal to the literal algebraic action, restricts continuously in both directions to the integral monoid, and satisfies the Galois identity and composition laws. In accordance with Absolute Anabelian Topics III, Section 0, the actual ind-object is now also retained: finite Galois integral stages form a filtered system of topological monoids, transitions are continuous and equivariant, and Krull evaluation is jointly continuous at every stage. Its exhaustive algebraic limit is O_(Kbar_v)^triangle and the stage-to-limit maps are injective and equivariant. The global number field is proved dense in K_v; primitive elements, same-degree polynomial approximation, root continuity, and Krasner's lemma show that every finite Galois stage is generated by a root of a global polynomial. Since the number field and each polynomial's root set are countable, these witnesses surject onto the stage type. Thus the countable-filtered-category theorem unconditionally constructs the paper's cofinal positive-integer presentation. The invariant sub-pseudo-monoid and continuous quotient descent are also proved. The cited Absolute Anabelian Topics II/III algorithms recovering these objects from the abstract D_v group, the rational fundamental group, and the infinity-kappa/infinity-kappa-times rational-function carriers remain absent.",
    ...
    clause "AbsTopIII.3.1(i-iii,v)" .absoluteAnabelianIII
      ...
      ["SourceTopologicalGroupMonoidActionPair",
        ...
        "SourceMLFGaloisComparison.continuous_autCongr",
        "SourceMLFGaloisComparison.continuousAutCongr",
        "SourceMLFGaloisComparison.separableClosureEquivAlgebraicClosure",
        "SourceMLFGaloisComparison.absoluteGaloisEquiv",
        "SourceMLFGaloisTMPair.monoAnalyticSeparable",
        "SourceMLFGaloisTMPair.pullbackActingGroup",
        "SourceThetaFiniteLocalCoreData.mlfGaloisTMPair",
        "SourceThetaFiniteLocalCoreData.xLocalGaloisTMPair",
    ...
    clause "II.1.4" .iutII "IUT II, Proposition 1.4"
      ["EtaleTheta.ThetaCenter",
        "EtaleTheta.thetaCenter_le_center",
        "EtaleTheta.thetaCenter_isMulCommutative",
        "EtaleTheta.LDeltaTheta",
        "EtaleTheta.lDeltaTheta_le_thetaCenter",
        "EtaleTheta.lDeltaTheta_le_center",
        "EtaleTheta.LDeltaThetaNumerator",
        "EtaleTheta.LDeltaThetaDenominator",
        "EtaleTheta.lDeltaThetaProjection_ker",
        "EtaleTheta.lDeltaThetaProjection_surjective",
        ...
        "GlobalLTorsionCoverInput.standardThetaRootContinuousMulEquiv",
        ...
        "GlobalLTorsionCoverInput.standardModLThetaCenterContinuousMulEquivCenterImage",
        "GlobalLTorsionCoverInput.standardThetaCenterImageEquiv",
    ...
    clause "II.2.7(ii)" .iutII "IUT II, Definition 2.7(ii)"
      ["TopologicalGroupCat.subgroupInclusion",
        ...
        "GlobalLTorsionCoverInput.standardModLThetaCenterNumerator_eq_center_preimage",
        "GlobalLTorsionCoverInput.standardModLThetaCenterContinuousMulEquivCenterImage",
        "ProfiniteFundamentalExactSequence.localGaloisSubquotient",
        "ProfiniteFundamentalExactSequence.localGalois_denominator_eq_geometric_range_comap",
        "ProfiniteFundamentalExactSequence.localGaloisContinuousMulEquiv",
        "ProfiniteFundamentalExactSequence.localGaloisContinuousMulEquiv_projection",
    ...
    clause "II.2.8(i)" .iutII "IUT II, Corollary 2.8(i)"
      ["ContinuousGroupAction", "ContinuousOneCocycle",
        ...
        "SourceThetaFiniteLocalCoreData.xLocalGaloisCoefficientAction",
        "SourceThetaFiniteLocalCoreData.xLocalGaloisH1Restriction",
        "SourceThetaFiniteLocalCoreData.xLocalGaloisRestrictedMonoidKummer",
        "SourceThetaFiniteLocalCoreData.xLocalGaloisRestrictedMonoidKummer_commutes",
        "SourceThetaFiniteLocalCoreData.xLocalGaloisRestrictedUnitKummer",
        "SourceThetaFiniteLocalCoreData.xLocalGaloisRestrictedUnitKummer_commutes"]
      .partialImplementation
      "Continuous nonabelian H1 is a quotient of actual continuous cocycles, and the displayed colimit over open subgroups is an explicit germ quotient. For abelian coefficients, pointwise cocycle multiplication descends to commutative group structures on H1 and its germ colimit; both restriction operations are proved homomorphisms. Two equivariant restriction stages derive the final mu_2l/mu orbit families indexed by |F_l|, preserve identity representatives at the zero label, and prove that the composite restriction sends every ambient orbit value into its final orbit. At the finite local endpoint, the cyclotomic coefficient action is pulled back across the constructed G_v quotient equivalence, the induced H1 restriction is an actual monoid homomorphism, and the monoid and unit Kummer restriction squares commute. Constructing the restricted-tempered and decomposition-group maps, connecting their coefficient actions and input orbits to this endpoint from the mono-theta environment, and proving the required orbit equivariance remain open.",
    ...
    clause "II.2.8(iii)" .iutII "IUT II, Corollary 2.8(iii)"
      ["SourceIUTIIUnitKummerEmbedding.constantSubgroup",
        ...
        "SourceThetaFiniteLocalCoreData.unitKummerEmbedding",
        "SourceThetaFiniteLocalCoreData.unitKummer_injective",
        "SourceThetaFiniteLocalCoreData.xLocalGaloisUnitKummerEmbedding",
        "SourceThetaFiniteLocalCoreData.xLocalGaloisUnitKummer_injective",
        "SourceIUTIICorollary28RestrictionStages.afterSecond_zero_representatives"]
      .partialImplementation
      "M_TM^x is now the range of an injective unit Kummer homomorphism, and its torsion M_TM^mu is proved exactly equal to the image of torsion units. The literal mono-analytic absolute-Galois TM-pair and coefficient-identity Kummer embedding are source-constructed, and their full monoid and unit maps reduce definitionally to the compatible-root Kummer map. At every finite Initial Theta core, the completion's canonical discrete valuation is proved compatible with its topology and makes it an MLF; the separable/algebraic closure comparison constructs this embedding on the core's exact local absolute Galois group. The pair is then transported to the arithmetic fundamental-group quotient by the geometric image, where its canonical unit Kummer map is again proved injective. A pointed inversion has an actual decomposition subgroup, and the exact equality M_TM^x intersect res^-1(torsion)=M_TM^mu derives the weaker detection implication and splitting. For both finite mu_2l and infinite mu orbits, the identity representative and finite-order action prove the entire final zero-label orbit is torsion; the composite H1 restriction constructs both zero-label splittings. Constructing the restricted-tempered map to this local MLF quotient and its coefficient comparison, proving the pointed inverse-image equality from Corollary 1.12, and identifying the two-stage composite with that pointed restriction remain open.",
