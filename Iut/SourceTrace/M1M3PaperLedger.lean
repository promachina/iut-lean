  ...
  [ clause "FrdI.1.1(i-ii)" .frobenioidsI
    ...
    clause "II.4.10(iv)" .iutII "IUT II, Corollary 4.10(iv)"
      ["SourceTheorem311TimesMuPrimeStripConstruction.toFamily",
        "SourceTheorem15TimesMuHorizontalCoricity",
        "SourceTheorem15TimesMuHorizontalCoricity.thetaLink_full",
        "SourceTheorem15TimesMuHorizontalCoricity.toTriangleSquare",
        "SourceTheorem311EnvironmentMonoAnalyticPrimeStripFamily.toTimesMu",
        "SourceFTimesMuReconstructionAlgorithm.mapFullPolyIsomorphism_inverse",
        "ToyIUTIIHodgeArakelovEvaluationData"] .partialImplementation
      "One reconstruction algorithm computes every site, vertically coric, and environment times-mu endpoint and derives the environment comparisons from mono-analytic Proposition 2.1(vi) data. The source record states the exact Section 0 fullness claim separately from functoriality: every times-mu horizontal class has a mono-analytic theta-link preimage. Lean derives a canonical theta-link member through the common source model; given the fullness theorem boundary and the two-sided Theorem 1.5 Kummer sources, it reconstructs the horizontal and Kummer maps and derives the complete clause (a) square. Constructing the fullness and Kummer source data from Corollaries 4.5, 4.6, and 4.10 remains open.",
    ...
    clause "III.3.11(ii).objects" .iutIII "IUT III, Theorem 3.11(ii): tensor and Frobenioid compatibility"
      ...
      "Distinguished-submodule compatibility is derived from equality of generating pure-tensor sets and `map_span`. Finite-place full and distinguished integral-region compatibility is derived from the actual log-shell lattices, including both closures. Infinite-place full and distinguished integral-region compatibility is derived from transported Hermitian pairings on arbitrary packet tensors. The assembled column boundary requires each site splitting action, its Kummer maps, a multiplicative realization of its actual unit group in the vertical log-link domain, and the roots-only adjacent-overlap theorem, but accepts no arbitrary bad-prime relation. It also requires the labeled profinite, number-field, and Frobenioid maps against the same common presentation. Constructing these inputs from IUT II Corollaries 4.6 and 4.8, together with the MOD/mod comparison squares, remains open.",
    ...
    clause "III.3.11(iii).horizontal" .iutIII "IUT III, Theorem 3.11(iii): horizontal theta link"
      ...
      .partialImplementation
      "The active boundary is an adjacent-column corridor, not a field of one column. It proves that the columns use the same lattice and have indices n and n+1. Following IUT I Section 0, the ordinary F, mono-analytic F, and Definition 4.9 F-turnstile-times-mu full poly-isomorphisms are quotients of all complete componentwise equivalence maps by structured natural isomorphism, not strict categorical Iso records. Clauses (a) and (b) use only times-mu corners computed by one reconstruction algorithm. The corridor no longer accepts a clause (a) square or a chosen theta-link class: Lean derives the latter through the endpoints' common model, while the source record supplies Corollary 4.10(iv)'s explicit surjectivity onto every horizontal times-mu class and the two-sided Theorem 1.5 Kummer sources. Lean maps these data through reconstruction, defines the lower horizontal class by Kummer conjugation, and proves commutativity and both directions of full-poly compatibility; clause (b) is then derived by environment conjugation. Constructing the stated fullness and Kummer source data from the preceding local theorems remains open. Clause (c) is a square of actual mono-theta projective systems for every m and selected bad place. Clause (d) is a kappa-sol/M-infinity-kappa square for every m and absolute label, plus the common square and coherence with both vertical labeled Kummer maps. Evaluation equivariance also remains open.",
