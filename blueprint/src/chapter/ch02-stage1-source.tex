...
\begin{proposition}[Localized Step (xi) \(C_\Theta\) source]
  ...
  \leanok
  The current strongest valuation-cover boundary starts from a named localized
  Remark~3.9.5 vector-bundle determinant source.  Its weighted adjusted local
  terms give the Step~(xi) finite sum for the canonical \(C_\Theta\)-scale,
  while the local arithmetic upper term is decomposed into the Step~(xi) term,
  a Haar normalization defect, and a main-log term.
\end{proposition}

\begin{proof}
  Lean identifies this localized source with the formula-matching Step~(xi)
  source used by the local arithmetic corridor, derives the displayed
  Ob3/Ob4 and valuation-cover handoffs, and then invokes the finite-sum closed
