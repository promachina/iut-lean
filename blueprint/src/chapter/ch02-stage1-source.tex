...
\begin{definition}[Continuous theta-value germs and Gaussian monoids]
  ...
  (operatorname{Ism})-orbit is derived.  The archimedean natural-system maps
  and the assembled prime-strip full poly-isomorphism category remain open.
...
\begin{definition}[Source-shaped Theorem 3.11 interfaces]
  ...
  every square.  The present prime-strip square is an ordinary-\(F\)
  precursor, not yet clause~(iii)(a), whose corners in the source are the
  Definition~4.9 \(F^{\vdash\times\mu}\)-prime-strips.  Following the
  convention of IUT~I, Section~0, an ordinary \(F\)-prime-strip full
  ...
  classes; its chosen upper and lower classes only witness nonemptiness.  Thus
  one convenient commuting representative cannot discharge a full
  poly-isomorphism compatibility claim.  The triangle
  ...
  facts validate the coarsification and conjugation mechanism but cannot
  discharge clauses~(a)--(b) until the new finite/archimedean
  \(F^{\vdash\times\mu}\)-prime-strip records have been populated from the
  source reconstruction theorems and their structure-preserving isomorphism
  category has been constructed.  The
  mono-theta square ranges over every \(m\) and bad
  place and uses actual projective systems with levelwise environment
  realizations.  The \(\kappa\)-sol/\(M_\infty^\kappa\) squares range over every
  \(m\) and absolute label and commute with the common square through the two
  vertical labeled Kummer maps.  These fixed-corner squares transport the
...
\begin{definition}[Assembled source Theorem 3.11 column and corridor boundaries]
  ...
  \lean{Iut.SourceTheorem311HorizontalCorridorBoundary}
  \lean{Iut.SourceTheorem311HorizontalCorridorBoundary.siteKappaClauseD_commutes}
  \leanok
  ...
  squares and their coherence with vertical Kummer.
