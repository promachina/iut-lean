...
## Math Milestone 101: Split Hull+Det into Hull and Determinant/Log-Volume Steps
...
Mochizuki's 2026 formalization report describes the third triangle as

```text
(HDD) := (hull+det) o (dsc)
```

and explains `hull+det` as obtained essentially by taking the determinant of
a module over a ring generated by a topological module appearing in the
multiradial representation. IUT III also describes Corollary 3.12 in terms of
the log-volume of the holomorphic hull of the union of possible images of the
Theta-pilot object, subject to `(Ind1)`, `(Ind2)`, `(Ind3)`.

Scholze-Stix's criticism of the proof of Corollary 3.12 focuses on whether the
relevant real-line identifications and pilot-object comparisons are actually
available. This milestone does not settle that issue, but it prevents the
formalization from treating "hull+det" as an indivisible inequality-producing
black box.

### Lean/API Check

`AlgorithmicOutput` now has the unmeasured abbreviation:

```text
AlgorithmicOutput.CommonTargetHull
```

`AlgorithmicBridge` now defines:

```text
AlgorithmicOutput.StructuredCommonHullBridge
AlgorithmicOutput.StructuredDeterminantLogVolumeBridge
AlgorithmicOutput.StructuredHullDetBridgeData
StructuredHullDetBridgeData.StepAudit
```

The internal step audit exposes:
...
thetaToyStructuredHullDet_determinant_volume_bound
```

The older toy `StructuredCommonTargetHullBridge`, `HullDetHullBridgeData`, and
`HullDetBridgeData` are now derived from this split data.

### Lean Decisions

The determinant/log-volume bridge is parameterized by a previously supplied
common-hull bridge:
...
This dependency matters. It prevents the determinant/log-volume estimate from
being stated for an unrelated region. The volume bound is explicitly a bound
for the hull produced by the hull bridge on the same structured certificate.

### What This Tests

Lean verifies that:

* a common hull can be built without measurement;
* a determinant/log-volume-style bound can be attached to that exact hull;
* the pair can be converted to the previously introduced measured common-hull
  bridge;
* downstream hull+det, HDD, SHE, common-container, and charted comparison code
  still consumes the derived common-target bridge.

Focused builds for `Iut.Foundations.AlgorithmicBridge` and
`Iut.Stage1.ToyBridge` pass.

### Design Trap Avoided

The trap would be to encode Mochizuki's phrase "hull+det" as a single axiom or
opaque bridge. The formal code now asks two separate questions:

```text
Which common hull is constructed?
Why is the log-volume/determinant measure of that hull bounded?
```

That distinction is directly relevant to the Corollary 3.12 dispute.

### Remaining Gap

The toy determinant/log-volume bound still comes from upper-ray normalization.
The actual IUT content remains to replace the toy upper-ray hull with a
